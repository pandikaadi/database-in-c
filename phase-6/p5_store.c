...
uint32_t create_new_page(FILE *file, uint8_t is_leaf) {
    ...
    Page page;
    init_page(&page, page_num, is_leaf);  // Initialize page in memory
...
int insert_into_page(Page *page, uint32_t key, uint32_t value) {
    if (page->num_keys >= MAXIMUM_KEYS) return 0; // Page is full
